...
@section('page-header')
    ...
    <div class="breadcrumb-header justify-content-between">
        <div class="my-auto">
            <div class="d-flex">
                <h4 class="content-title mb-0 my-auto">{{ __('panel.manage_document_types') }}</h4>
        ...
        <div class="d-flex my-xl-auto right-content">
            ...
            <div class="pr-1 mb-3 mb-xl-0">
                ...
                <a href="{{ route('admin.document_types.index') }}" class="btn btn-warning  btn-icon ml-2"><i
            ...
            <div class="card">
                <div class="card-header pb-0">
                    <div class="d-flex justify-content-between">
                        ...
                        @ability('admin', 'create_document_types')
                            ...
                            <tbody>
                                ...
                                @foreach ($documentTypes as $docType)
                                    <tr>
                                        ...
                                        <td>{{ $docType->documentCategory->doc_cat_name }}</td>
                                        <td>{{ $docType->created_at->format('Y-m-d ') }}</td>
                                        <td>{{ $docType->created_by }}</td>
                                        <td>{{ $docType->status() }}</td>
                                        <td>
                                            <div class="btn-group btn-group-sm">
                                                <a href="{{ route('admin.document_types.edit', $docType->id) }}"
                                                    class="btn btn-primary">
                                                    <i class="fa fa-edit"></i>
                                                </a>
                                                <a href="javascript:void(0);"
                                                    onclick=" if( confirm('{{ __('panel.confirm_delete_message') }}') ){document.getElementById('delete-document-categories-{{ $docType->id }}').submit();}else{return false;}"
                                                    class="btn btn-danger">
                                                    <i class="fa fa-trash "></i>
                                                </a>
                                            </div>
                                            <form action="{{ route('admin.document_types.destroy', $docType->id) }}"
                                                method="post" class="d-none"
                                                id="delete-document-categories-{{ $docType->id }}">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                                        </td>
